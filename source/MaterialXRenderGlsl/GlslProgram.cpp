...
const GlslProgram::InputMap& GlslProgram::updateUniformsList()
{
    ...
    if (_shader)
    {
        ...
        for (size_t i = 0; i < constants.size(); ++i)
        {
            ...
            // todo - add type strng to ShaderPort
        ...
        for (const auto& uniformMap : ps.getUniformBlocks())
        {
            ...
            for (size_t i = 0; i < uniforms.size(); ++i)
            {
                ...
                const auto populateUniformInput =
                    ...
                    (TypeDesc typedesc, const string& typeName, const StructMemberDescVec* variableStructMembers, const string& variableName, ConstValuePtr variableValue) -> void
                {
                    auto populateUniformInput_impl =
                        [this, variablePath, variableUnit, variableColorspace, variableSemantic, &errors, uniforms, &uniformTypeMismatchFound]
                        (TypeDesc typedesc, const string& typeName, const StructMemberDescVec* variableStructMembers, const string& variableName, ConstValuePtr variableValue, auto& populateUniformInput_ref) -> void
                    {
                    ...
                    return populateUniformInput_impl(typedesc, typeName, variableStructMembers, variableName, variableValue, populateUniformInput_impl);
                };
