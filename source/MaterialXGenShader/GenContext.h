...
class MX_GENSHADER_API GenContext
{
  public:
    ...
    void registerCustomTypeDesc(const string& name, uint8_t basetype, uint8_t semantic = TypeDesc::SEMANTIC_NONE, uint8_t size = 1, ConstStructMemberDescVecPtr structMembers = nullptr);
    ...
    const StringVec& getCustomTypeDescNames() const;
  ...
  private:
    ...
    vector<std::shared_ptr<const TypeDesc::DataBlock>> _typeDescDataBlockStorage;
    ...
    StringVec _customTypeDescOrder;
};
